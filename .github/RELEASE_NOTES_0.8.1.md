# VaultHub v0.8.1 — 缓存与媒体库体验修复

## 更新内容

- 新增独立转码缓存目录设置，支持自定义目录、最大字节数、最大保留时间和清理周期。
- 缓存启动时清理过期文件，并按最旧优先策略控制总容量，避免系统盘被 FFmpeg 缓存占满。
- 音乐播放器在没有活动播放项目时隐藏；停止、播放列表结束和切换页面时正确释放显示状态。
- 移除媒体内容页顶部重复标签信息。
- 漫画/电子书按媒体库和子类型独立展示，支持从媒体库导航切换，不再通过顶部标签切换。
...
services:
  vaulthub:
    image: ghcr.io/q807738511/vaulthub:v0.8.1
    environment:
...
保留 `/data` 持久卷；生产环境建议将 `MEDIA_CACHE_DIR` 指向容量充足的数据卷，并固定使用版本标签。