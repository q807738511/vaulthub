# VaultHub 蜀鼠之家 v0.8.4：媒体详情与运行设置

v0.8.4 增加刮削来源及缓存的 WebUI 运行配置，修复漫画/电子书侧栏切换，并重构视频海报、详情页、播放器布局和媒体元数据显示。环境变量继续作为首次启动默认值，WebUI 保存值持久化到 `/data/media-runtime.json` 并立即生效。
