# VaultHub 蜀鼠之家 v0.9.40：视频播放器悬浮控制栏

v0.9.40 重做视频播放器控制栏：播放时滑动鼠标唤出，3 秒无操作自动隐藏。左上角 `⌄` 向下折叠（折叠后左下角出现 `⌃` 展开），右上角 `⛶` 全屏切换；左下角显示剧集集数与分集标题（电影显示年份）以及「当前时间 / 视频时长」；中部依次是上一个、快退、暂停/播放、快进、下一个、关闭播放；右下角依次是更多操作（含获取信息）、重复播放、随机播放、设置（转码质量 / 音频流 / 字幕）、播放列表、声音；进度条横跨底部方框上方，支持点击定位与拖动。

镜像：`ghcr.io/q807738511/vaulthub:v0.9.40`（Docker Hub 同步 `q807738511/vaulthub:v0.9.40`）。
...
- **折叠**：左上角 `⌄` 收起整条控制栏，左下角出现 `⌃`。折叠期间滑动鼠标不再唤出 —— 折叠是显式意图
- **全屏**：优先整块播放区全屏（控制栏一起进全屏），浏览器拒绝时回落到 `<video>` 元素
...
设置浮层新增 自动 / 原画直放 / 1080p / 720p / 480p。显式选择分辨率时后端强制重新编码
...
影视库列表与剧集详情会把当前排序后的文件写入播放队列，「上一个 / 下一个 / 播放列表」在同一队列内跳转。
