...
4. Apple 风格居中播放器：海报 / **歌词**双页切换，歌词逐行高亮、随播放自动滚动、点击跳转；放大海报后右侧另有「🖼 海报 / 📜 歌词」药丸，歌词在页面中部展示、可拖动浏览（松手 6 秒内暂停自动跟随）、点击任意行跳到该句播放，主海报按宽度 5% 虚化衬托歌词
5. **特殊字符 / 日语 / 全角标题的识别策略**：iTunes 店铺按查询脚本动态选择（假名→JP 优先）、检索词阶梯（原样→去修饰→主歌手→仅标题）、全角与半角统一折叠；歌手封面用 MusicBrainz 别名表桥接罗马音规范名（`高橋洋子 → Yoko Takahashi`）；歌词取源同样走阶梯（本地 `.lrc`/内嵌标签 → LRCLIB 多变体 → 自由文本兜底 → lyrics.ovh），并拒绝「匹配不上就贴第一条有歌词记录」的旧行为
6. 音乐刮削失败自动以文件名展示，不影响浏览与播放
7. 音乐与视频持续播放时自动维持登录会话，长时间无点击/滚动也不会在 30 分钟后被登出

### 📶 弱网与远程访问（v0.9.71）

参考 Plex / Emby / Navidrome 的通行做法，给「远程访问、4G、家里 Wi-Fi 边缘」提供一套本容器可直接用的方案，
完整客户向手册见 [`docs/weak-network-playbook.md`](docs/weak-network-playbook.md)：

- **音频限码率转码流** `/api/media/audio/stream?bitrate=128k`（档位 64/96/128/160/192/256/320）：
  ffmpeg 转 MP3 后落盘 `<MEDIA_CACHE_DIR>/audio-transcode/`，命中缓存走 `http.ServeContent`
  —— 支持 Range（可拖进度）与 1 天私有缓存，重播不再重复转码；缓存配额独立于漫画页缓存。
- **下行自动测速** `/api/media/weak/probe?kb=512`：不可压缩数据 + 禁压缩/禁缓存，前端据此判定
  `弱网（<400 KiB/s）` / `中等（<1.5 MiB/s）` / `良好`，30 分钟内复用。
- **客户端档位**：播放器「音质·原文件」药丸一键在
  `原文件 → 自动 → 320k → 192k → 128k → 96k` 之间切换（显式选择优先，切换保持播放位置）；
  系统设置里有「弱网模式（自动/始终/关闭）」与一键测速；弱网判定同时驱动漫画「省流」自动开启。
- **不卡死**：转码队列拥塞返回 `503 + Retry-After`，前端自动回落原文件直出；转码共用全局闸门，不会打满 CPU。
