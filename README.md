# VaultHub 蜀鼠之家 v0.6.31：Plex 界面、登录状态与视频缓存加速

v0.6.31 把 `v0.6.30.Branch-update` 分支的 Plex 风格界面改版合入 main，并在其基础上补齐退出登录、
Caddy 独立配置页、真实显卡检测、登录状态监测和视频缓存流式起播。
...
```
ghcr.io/q807738511/vaulthub:<版本标签>   # 例如 v0.6.31
ghcr.io/q807738511/vaulthub:latest        # 随 main 漂移，生产不建议
```

**建议固定版本标签部署（如 `v0.6.31`），不要用 `latest`**：`latest` 会随每次推送变化，出问题难回滚。
...
services:
  vaulthub:
    image: ghcr.nju.edu.cn/q807738511/vaulthub:v0.6.31
```

> 注意只有一个冒号：`vaulthub:v0.6.31`，不是 `vaulthub::v0.6.31`。

用法二——保持 compose 用官方名，手动拉取后打回原名（便于随时切回直连）：

```bash
docker pull ghcr.nju.edu.cn/q807738511/vaulthub:v0.6.31
docker tag  ghcr.nju.edu.cn/q807738511/vaulthub:v0.6.31 ghcr.io/q807738511/vaulthub:v0.6.31
# compose 仍写 image: ghcr.io/q807738511/vaulthub:v0.6.31
docker compose up -d
```
