...
NOTES = (ROOT / ".github/RELEASE_NOTES_0.9.61.md").read_text(encoding="utf-8")
...
# ============ T1 版本一致性（bump 0.9.59 → 0.9.61） ============
check("T1 HTML 资源版本", 'VAULTHUB_ASSET_VERSION = "0.9.61"' in HTML)
check("T1 JS 脚本版本", 'VAULTHUB_SCRIPT_VERSION = "0.9.61"' in STATE)
check("T1 CSS 缓存串", 'href="/web/css/main.css?v=0.9.61"' in HTML)
check("T1 JS 缓存串 x5", HTML.count('?v=0.9.61') >= 6)  # css 1 + js 5
check("T1 无 0.9.59 缓存串残留", HTML.count('?v=0.9.59') == 0)
check("T1 UI 版本角标", "v0.9.61" in HTML)
...
# ============ T4 文档一致性 ============
check("T4 Update Log v0.9.61 段", "# VaultHub 蜀鼠之家 v0.9.61" in UPDATELOG)
check("T4 Update Log 编辑跳转记载", "编辑按钮跳转" in UPDATELOG or "跳转曲目列表" in UPDATELOG)
check("T4 RELEASE_NOTES 存在", "v0.9.61" in NOTES and "移除旧最大化按钮" in NOTES)

if fails:
    print(f"FAIL: v0.9.61 契约 {len(fails)} 项未通过")
    for f in fails:
        print("  -", f)
    raise SystemExit(1)

print("PASS: v0.9.61 契约全部通过")
