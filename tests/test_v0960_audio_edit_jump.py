...
check("T1 HTML 资源版本", 'VAULTHUB_ASSET_VERSION = "0.9.68"' in HTML)
check("T1 JS 脚本版本", 'VAULTHUB_SCRIPT_VERSION = "0.9.68"' in STATE)
check("T1 CSS 缓存串", 'href="/web/css/main.css?v=0.9.68"' in HTML)
check("T1 JS 缓存串 x5", HTML.count('?v=0.9.68') >= 6)  # css 1 + js 5
check("T1 无 0.9.59 缓存串残留", HTML.count('?v=0.9.59') == 0)
check("T1 UI 版本角标", "v0.9.68" in HTML)
...
# ============ T4 文档一致性 ============
check("T4 Update Log v0.9.67 段", "# VaultHub 蜀鼠之家 v0.9.68" in UPDATELOG)
check("T4 Update Log 编辑跳转记载", "编辑按钮跳转" in UPDATELOG or "跳转曲目列表" in UPDATELOG)
check("T4 RELEASE_NOTES 存在", "v0.9.68" in NOTES and "歌词" in NOTES)
