...
# v0.9.80：vaulthub.env 已停用，compose 不再引用 env_file（配置迁移到 Web → /data/media-runtime.json）。
...
# 版本号
check("index.html 当前版本号为 v0.9.73",
      read("index.html").count("v0.9.73") >= 2,
      f"出现 {read('index.html').count('v0.9.66')} 次")
