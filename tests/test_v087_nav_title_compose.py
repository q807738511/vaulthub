...
# compose 必须引用 env_file（v0.9.55 起为可选覆盖层：path + required: false）
...
# v0.9.55：compose 跟随 GHCR 可写 latest（每次正式版本 tag 与版本号同 digest）
check("compose 跟随 GHCR latest（v0.9.55 起）",
...
# env_file 在 v0.9.55 已是可选（required: false，缺失不报错）；历史安装/升级脚本
...
check("index.html 版本号为 v0.8.7",
      read("index.html").count("v0.9.55") >= 2,
      f"出现 {read('index.html').count('v0.9.55')} 次")
