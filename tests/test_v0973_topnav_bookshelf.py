...
NOTES = (ROOT / ".github/RELEASE_NOTES_0.9.80.md").read_text(encoding="utf-8")
...
check("A3 顶栏一级导航（首页 / PT）—— v0.9.80 移除媒体搜索按钮（右侧已有搜索框）",
...
check("A7 独立脚本 07-topnav.js 已挂载且在主题引擎之后",
      "/web/js/07-topnav.js?v=0.9.80" in HTML
...
# ---------------------------------------------------------------- B. 书刊展示页
check("B1 三个书架标签（书架/喜欢/历史阅读）—— v0.9.80 未读改名书架、移除全部",
...
for token, value in [("--topbar-h", "52px"), ("--sidebar-w", "236px"), ("--ebook-font-size", "17px")]:
    check(f"C 布局令牌 {token} 已定义（{value}）",
          ...
          f"v0.9.80 曾漏掉 {token}，顶栏/侧栏会当场塌掉")
...
# ---------------------------------------------------------------- D. 文档与版本
check("D1 发布说明存在且包含视频推荐与评分配置（v0.9.80 说明随版本推进）",
      "影视" in NOTES and ("分享" in NOTES or "五星" in NOTES or "评分" in NOTES))
check("D2 Update Log 有对应条目", "顶栏导航" in LOG and "v0.9.80" in LOG)
check("D3 版本号为 0.9.80",
      'VAULTHUB_ASSET_VERSION = "0.9.80"' in HTML and 'VAULTHUB_SCRIPT_VERSION = "0.9.80"' in STATE
      and "?v=0.9.80" in HTML and "?v=0.9.78" not in HTML)
...
if fails:
    raise SystemExit("v0.9.80 契约测试未通过")
print("v0.9.80 顶栏导航 + 书刊展示页契约测试通过")
