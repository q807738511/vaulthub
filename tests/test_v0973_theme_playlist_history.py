#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""v0.9.80 契约测试：播放模式提示遮挡 / 播放列表 / 历史阅读释放 / 主题三层叠加。
...
NOTES = (ROOT / ".github/RELEASE_NOTES_0.9.80.md")
...
# ---------------- 版本与发布资料 ----------------
check("V 入口页资源版本 0.9.80", f'window.VAULTHUB_ASSET_VERSION = "{VERSION}";' in HTML)
check("V 脚本自校验版本 0.9.80", f'const VAULTHUB_SCRIPT_VERSION = "{VERSION}";' in STATE)
check("V 所有静态资源带 ?v=0.9.80", HTML.count(f"?v={VERSION}") >= 8, f"命中 {HTML.count(f'?v={VERSION}')}")
check("V 主题脚本被入口页引用", f'/web/js/06-theme.js?v={VERSION}' in HTML)
# 同一版本内角标文案随本版主功能追加而调整（v0.9.80 收尾加入顶栏导航与书刊页）。
check("V 侧栏与关于弹窗角标已升版", HTML.count(f"v{VERSION}") >= 1)
check("V release notes 文件存在", NOTES.is_file())
check("V Update Log 有 v0.9.80 段", f"# VaultHub 蜀鼠之家 v{VERSION}" in UPDATELOG)
...
if failed:
    raise SystemExit("FAIL: v0.9.80 契约测试未通过")
print("PASS: v0.9.80 契约（提示遮挡 / 播放列表 / 历史阅读释放 / 三层主题）全部满足")
