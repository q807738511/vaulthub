...
check("T1 HTML 资源版本", 'VAULTHUB_ASSET_VERSION = "0.9.69"' in HTML)
check("T1 JS 脚本版本", 'VAULTHUB_SCRIPT_VERSION = "0.9.69"' in STATE)
check("T1 CSS 缓存串", 'href="/web/css/main.css?v=0.9.69"' in HTML)
check("T1 JS 缓存串 x5", HTML.count('?v=0.9.69') >= 6)  # css 1 + js 5
check("T1 无 0.9.56 缓存串残留", HTML.count('?v=0.9.56') == 0)
check("T1 UI 版本角标", "v0.9.69 · Hardening R2" in HTML)
...
check("T4 Update Log v0.9.67 段", "# VaultHub 蜀鼠之家 v0.9.69" in UPDATELOG)
