...
checks = {
    ...
    # v0.9.79.1：书架视图从「两态按钮」升级为四个分段标签（未读 / 喜欢 / 🕘 历史阅读 / 全部），
    ...
    "PDF 使用应用内阅读器（v0.9.79.1：不再走浏览器内置查看器）": 'ext === "pdf"' in MEDIA and '/api/media/pdf/info' in MEDIA
    ...
    # v0.9.79.1：原契约是「关闭阅读器强制回到未读视图」；用户报告从「历史阅读」点开一本
    ...
    "本版版本": 'VAULTHUB_ASSET_VERSION = "0.9.79.1"' in HTML and 'VAULTHUB_SCRIPT_VERSION = "0.9.79.1"' in STATE,
}
...
checks.update({
    ...
    # 遗留 3：v0.9.79.1 起 PDF 由服务端 pdftoppm 渲染进应用内阅读器，不再走浏览器内置查看器
