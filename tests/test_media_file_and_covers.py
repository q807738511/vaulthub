...
assert "src[i]=='+'?' ':src[i]" in source, "query decoder does not preserve URLSearchParams spaces"
assert 'url.searchParams.set("path", String(path))' in html, "frontend still places book path in URL path segments"
