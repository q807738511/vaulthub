...
check(HTML.count("v0.9.59") >= 2, "关于与侧栏版本必须是 v0.9.59")
check('VAULTHUB_ASSET_VERSION = "0.9.59"' in HTML, "资源版本必须是 0.9.59")
check('VAULTHUB_SCRIPT_VERSION = "0.9.59"' in STATE, "脚本版本必须是 0.9.59")
...
if failures:
    print(f"FAIL: {len(failures)} 项 v0.9.59 契约未满足")
    for item in failures:
        print("  -", item)
    sys.exit(1)
print("PASS: v0.9.59 点击即播、计划超时、引擎隐藏、>256MB 不软解、静音删除、剧集播放列表门控与自动连播、最小化小窗、标题移入左上角、异常兜底")
