...
assert 'if (key === "account") { refreshSessionStatus(false); loadCaddyConfig(); loadAccountCredentialsUI(); }' in STATE, \
    "进入账户与登录页必须同时刷新会话状态与 Caddyfile，并加载 v0.9.56 登录凭据/鉴权模式面板"
...
# v0.9.76：书刊页改成自己的页头（pn-head，标题同样是 lib.name 动态渲染），
...
assert "<h1>${esc(lib.name)}</h1>" in JS, "书刊页标题必须使用库名称"
...
# ---------------------------------------------------------------- 版本与发布引用
assert "v0.9.76" in HTML and HTML.count("v0.9.76") >= 2, "关于与侧栏版本必须是 v0.9.76"
assert 'VAULTHUB_ASSET_VERSION = "0.9.76"' in HTML, "资源版本必须是 0.9.76"
assert 'VAULTHUB_SCRIPT_VERSION = "0.9.76"' in STATE, "脚本版本必须是 0.9.76"
