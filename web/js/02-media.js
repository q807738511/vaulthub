...
function buildSeriesShows(files) {
  ...
  files.forEach(file => {
    ...
    if (!shows.has(key)) shows.set(key, { key, title, poster: meta.poster || "", logo: meta.logo || "", fanart: meta.fanart || "", backdrop: meta.backdrop || "", overview: meta.overview || "", year: meta.year || "", provider: meta.provider || "文件名展示", seasons: new Map(), files: [] });
    const show = shows.get(key); show.poster ||= meta.poster || ""; show.logo ||= meta.logo || ""; show.fanart ||= meta.fanart || ""; show.backdrop ||= meta.backdrop || ""; show.overview ||= meta.overview || ""; show.year ||= meta.year || "";
