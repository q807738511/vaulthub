...
function buildSeriesShows(files) {
  ...
  files.forEach(file => {
    ...
    if (!shows.has(key)) shows.set(key, { key, title, poster: meta.poster || "", backdrop: meta.backdrop || "", overview: meta.overview || "", year: meta.year || "", provider: meta.provider || "文件名展示", seasons: new Map(), files: [] });
    const show = shows.get(key); show.poster ||= meta.poster || ""; show.backdrop ||= meta.backdrop || ""; show.overview ||= meta.overview || ""; show.year ||= meta.year || "";
...
function readSeriesShow(libId, showKey) { try { return JSON.parse(localStorage.getItem(seriesShowStoreKey(libId, showKey)) || "null"); } catch(e) { return null; } }
...
function openSeriesDetails(libId, showKey) { const lib=findMediaLibrary(libId), viewer=document.getElementById("local-media-viewer-movie"); if(!lib||!viewer)return; const show=readSeriesShow(libId,showKey); if(!show)return; const hero={title:show.title,overview:show.overview||"已按 Plex / Emby 风格根据根目录、Season 01 和 S01E01 规则聚合到同一剧集。",poster:show.poster,backdrop:show.backdrop,year:show.year,provider:show.provider,media_type:"series"}; const seasons=(show.seasonList||[]).map(season=>`<section class="series-season-block"><h3>Season ${String(season.season).padStart(2,"0")}</h3><div class="media-file-list">${season.episodes.map(ep=>renderSeriesEpisodeRow(lib,ep)).join("")}</div></section>`).join(""); viewer.innerHTML=`<div class="media-reader-overlay movie-detail-page series-detail-page"><div class="movie-detail-scroll"><button class="media-reader-close" onclick="closeMovieDetails()">✕</button>${renderMovieHero(lib,show.files?.[0]?.path||"",hero)}<section><h3>剧集列表</h3><div class="hint">按标准命名规则聚合：根目录剧名 / Season 01 / 剧名 S01E01 标题；刮削先锁定主剧集，再将本地多季多集挂载到同一条目。</div></section>${seasons}</div></div>`; scrollViewerIntoView(viewer); }
