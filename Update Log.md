...
## 剧集/季/集展示升级 · 视频推荐略缩图
- **顶部导航与媒体库顶栏导航**保持与上版一致，电影/剧集/分享/评分等入口始终可达；本版在其上叠加剧集/季/集详情展示升级。
- **剧集 TMDB 评分补齐**：进入剧集详情即按标题解析出 TMDB 剧集 id 取 `vote_average` 显示在 hero 与「视频信息」，并回写元数据缓存；不再因刮削时未带评分而缺失。
- **多季剧集季度展示**：剧集详情默认先展示「季度」卡片（季度信息 + 季度略缩图，第一季默认展开，其余折叠）；点击季度标题展开/收起该季集列表，点击集卡片直接播放。
- **单集详情网格**：单集详情用「X 排 Y 列」网格展示该剧集略缩图，点击其它集可直接切换播放。
- **演职人员 / 视频推荐 / 视频信息**：剧集单集详情与电影详情一致展示演职人员、视频推荐、视频元数据；「视频元数据」统一改名为「视频信息」（电影与剧集同步）。
- **视频推荐略缩图卡片**：推荐从文字改为横版略缩图 + 标题卡片，默认横版；服务器内有该标题资源时标「本地可播」并允许点击播放，可播放的内容优先排在前面。
- **无资源回落**：服务器无该视频资源时点击推荐卡弹出「视频信息」窗口展示 TMDB 信息，日漫/剧集无简介时异步回落 Bangumi（bgm.tv）信息展示。
...
## 硬件配置与账户管理配置重整
- **硬件配置**（`setpanel-hardware`）：显卡加速、NAS 监控、转码缓存统一收进「硬件配置」标签，归档页缓存配额（漫画/PDF）、扫描最大深度也在此在线编辑。
- **账户管理**（`setpanel-account`）：当前登录状态、登录凭据与鉴权模式、反向代理服务域名、关于、退出登录全部归入「账户与登录」。

## 卡顿排查与优化（对照《VaultHub卡顿排查与优化指南》）
- **SQLite 调优**：DSN 增加 `cache_size=16MiB`、`mmap_size=256MiB`、`temp_store=MEMORY`（纯内存参数，对 1 万+ 行大库排序/搜索收益明显，与磁盘类型无关）。
- **搜索索引**：`files` 表新增 `idx_files_lib_lower_path(lib, lower(path))` 函数索引，为到库的候选集预过滤，减小全表扫描压力。
- **归档目录缓存**：ZIP/CBZ 中央目录 LRU 容量由 8 提升到 64，减少连续翻阅不同漫画时的反复「逐出→重新解析」。
- **转码排队超时**：8s → 15s，避免高峰瞬时 CPU 打满时过早放弃转码回落直出。
- **归档页缓存配额默认 16GiB**，且可在线调整（见硬件配置）。扫描最大深度可在线改（0=默认 32）。

## 配置迁移：vaulthub.env 已停用
- **vaulthub.env 文件停用**，docker-compose 不再引用 `env_file: vaulthub.env`。所有可调参数默认值内置进镜像。
- 运行时配置（刮削来源 / API 链接测速 / 代理 / 转码缓存 / 归档页缓存配额 / 扫描深度）统一在 Web「系统设置」在线编辑，写入 `/data/media-runtime.json` 并立即生效，无需重建容器。
- 涉及修改 `.env` 的部署指引均已迁移到 Web 可编辑位置，不再依赖 `vaulthub.env`。
...
- 修复影视推荐不生效：此前详情页只在 `tmdb_id` 存在且非「本地 NFO」时才取 TMDB 详情与推荐；豆瓣/文件名/本地 NFO 来源因没有 tmdb_id 被跳过，推荐永远不显示。
- 现在打开详情时若缺 tmdb_id 会先按标题回查 TMDB 取 id（多语种），拿到 id 后统一取 TMDB 评分 + credits + recommendations。TMDB 未命中/密钥无效/网络失败都会有可见的中文提示（不再显示空洞的「暂无视频推荐」）。

## 演职人员头像刮削插件 + 依赖生态补完（Metashark 适配器 / Plex NFO Agent，内置，不额外装容器）
- 内置 **Metashark 适配器**（Jellyfin 生态开源能力）：豆瓣/TMDB 适配、中文文件名解析、演员头像下载器，头像经本地 `/api/media/cast/avatar` 代理抓 TMDB 人物头像并落 /data，不直连 image.tmdb.org（弱网/代理/外部浏览器都更稳）。
- 内置 **Plex NFO Agent**（XBMCnfo 导入 + NFO 导出器生成带 `<thumb>` 头像字段）依赖生态随镜像内置，不需要额外容器。
- 刮削面板新增「演职人员图像刮削插件」选择（Metashark / Plex / TMDB / 关闭）；插件为内置，不额外安装容器。

## 分离刮削与硬件（配置归类重构）
- 原「刮削与硬件」拆成两个独立标签页：**硬件配置**（显卡加速 / 转码缓存 / NAS 监控）+ **刮削配置**（刮削来源 / API 网络链接测速 / 网络能效 / 演职人员图像刮削插件）。
- 显卡加速、转码缓存、NAS 监控 → 硬件配置并单独立项；刮削来源、API 网络链接测速、演职人员图像刮削插件 → 刮削配置。配置入口与 Caddy 反向代理/账户同属系统设置。

---

# VaultHub 蜀鼠之家 v0.9.77

## 弱网与自动画质 / 音质：去除手动音质转换按钮，改为后台自动测速与自动切换
- 去除播放器/放大视图里的「音质转换」手动按钮与档位循环；音质始终保持「自动」，由后台按测速结果限码率，无需手动操作。
- WEBUI 当天首次打开时自动测速：测得本地网络到容器的延迟（多次 1 KiB 取中位数）与下行带宽，结果写入弱网档位，自动辅助音频限码率、漫画省流与视频画质转换，达到最佳使用效果。
- 设置页网络测速区新增「自动测速」开关（默认开），可关闭或手动「立即测速」；增加音质当前档位状态文案。
- 视频播放计划携带当天测速带宽（network_bps），供服务端据此决定自动画质上限。

## PDF 漫画不再用浏览器内置阅读器（客户反馈修复）
- PDF（漫画或书籍）改由服务端 pdftoppm 渲染进应用内阅读器：单页/双页/条漫、翻页、右起/左起、省流按页转码与预取、阅读进度；不再交给浏览器内置 PDF 查看器。
- 服务端新增 PDF 页面信息与分页渲染端点（/api/media/pdf/info、/api/media/pdf/page、/api/media/pdf/cover），带页边界校验（页数上限、单页渲染超时、请求节流到既有限流闸门），第 3 项客户反馈的「PDF 用浏览器播放器」由此解决。
- PDF 封面用第 1 页小图（服务端渲染），书架不再依赖浏览器 PDF 缩略图。

## 视频自动画质 + 弱网修复（v0.9.76 遗留）
- 客户端把当天测速带宽传给 /play 计划，自动画质按带宽命中更高的码率/分辨率档，减少卡顿。
- 自动画质限高：自动档按实测带宽给码率上限，优于无差别拉最高；TMDB/带宽信息异常时回落安全档。

## 自动画质/可靠性
- 自动档视频画质默认（配合带宽）在良好带宽取最高、弱网下降档；测速失败/未测速回落原样，避免劣化。
- 转码拥塞、ffmpeg 缺失时视频/音频转码失败自动回落原文件，不让播放卡死。

## 服务端坚固（v0.9.77 审查）
- TMDB 响应里的密钥/错误正文在回传前端前擦除（不复述上游 secret、超长截断）；TMDB 调用自动带多语种查询参数；对 TMDB 超时/4xx 合并处理与逐条降级。
- 修复 TMDB v4 Read Access Token（JWT）鉴权：此前一律拼 ?api_key= 导致 401「TMDB 评分/影视推荐不生效」的根因（评分/推荐仍不生效已在 v0.9.76 修复；本次复核 TMDB 流量+节流+语义）。
- 上述修改包含审查要求的最少权限 / 运行时可靠性 / 配置与密钥 / 网络与数据 / 代码逻辑。
...
## 视频推荐（TMDB）
- 详情页「视频推荐」改为调用 TMDB recommendations，推荐线 = 当前视频评分 +5%（达标项按 TMDB 评分降序）。
...
## 影视详情四项精修（按用户反馈）
- 分享地址：内网默认用当前访问的 `IP:端口`；系统设置可配「影视分享外网地址」（IP/域名，服务端校验 http(s)://host[:port]），配置后分享走外网并做可达性探测（不可达仅提示）。分享修复：http 非安全上下文下 `navigator.share`/`navigator.clipboard` 不可用且旧实现静默吞错。改为三级降级（系统分享 → 剪贴板 → execCommand 兜底），每级有 toast 反馈；分享文案带库与路径定位。
- 十分制五星图示评分：评分不再弹输入框，hero 区五星图示（每星 2 分、半星步进），鼠标悬停跟踪预览、点击落定；黄色柔和五角星。hero 区新增 TMDB 评分徽标（⭐ + TMDB）。
- 收藏可取消：按钮 aria-pressed 同步 + 操作 toast；状态存本机。
- 演职人员改圆形头像 + 下方姓名/角色（TMDB profile_path，缺图首字占位）。
- 分享面板：链接可选中 + 一键复制按钮，绕过 http 下剪贴板权限限制；外网地址配置后先探测可达性。
- 清空外网分享地址：表单带 `share_public_base_set` 标记，清空保存即生效。

> 发布约定（v0.9.75 起）：GitHub 仓库 README 固定为项目介绍，不再随版本改动；升级日志一律由本文件承载。Docker Hub 上的仓库说明与日志与 GitHub 侧分开维护（CI 只推镜像与 GitHub Release，不镜像 README）。

# VaultHub 蜀鼠之家 v0.9.74

## 按用户反馈的界面精简（书架视图 + 顶栏）
- 顶栏移除「媒体搜索」一级导航按钮：右上角已有常驻搜索框，两处入口重复；搜索框行为不变。
- 书架标签改名并裁剪：「未读」→「书架」（扫描到的文件统一先进书架展示），移除「全部」标签；旧的 `bookShelfTab = "all"` 偏好自动回退到书架。
- 页头「全库」计数保留；筛选/排序/密度/分页行为不变。

# VaultHub 蜀鼠之家 v0.9.73

## 播放器：提示遮挡与播放列表
- 修复播放模式提示被覆盖：底部播放器（z-index 250）与最小化视频条（650）整条压住提示条（原 200）。提示条 z-index 抬到 400，并按底部浮层真实位置写入 `--toast-lift` 上移，弹出是动画而非瞬移。
- 播放器控制条的「歌曲详情与歌词」按钮改为「播放列表」：面板挂在播放器上方，显示当前队列、当前曲目高亮滚入视区、点行跳播、单项移出、一键清空、整队列保存为歌单。
- 队列仍以 `audioFiles` + 当前索引为唯一真相（连播/上一首/下一首/专辑/歌手/歌单共用），移出靠前曲目时索引左移一位避免跳歌；歌曲详情/歌词保留在每行 ⓘ 与曲目行 ✎。

## 历史阅读（原「已读收藏」）释放修复
- 统一改名：已读收藏 → 🕘 历史阅读，归档提示改为「已移入历史阅读」。
- 释放失效的根因有两条：服务端进度库只有写入没有删除入口（只能写 progress=0，条目仍在库里），且 localStorage 旧值会在下次读取时把状态翻回已读。
- 新增 `DELETE /api/media/reading/progress?id=&path=`：真删条目、幂等（无条目回报 removed=false）、沿用与写入相同的路径包含性校验、按库隔离；前端释放同时清服务端条目、内存缓存与 localStorage 残留，并撤掉未落盘的待写进度。
- 阅读器头部按钮改为两态（未读「✓ 标记已读」/ 已读「↩ 释放」），历史阅读视图每张卡片也有「↩ 释放」。
- 关闭阅读器不再强制跳回未读视图（否则从历史阅读点开一本书再关闭，看起来就像条目被释放）。

## 外观主题：调色板 × 明暗 × 强调色
- 主题升级为三层叠加（会话 mtkzexy5e4e2te 的方案）：4 套调色板（青瓷/午夜紫曜/霓虹胶片/暖陶）× 亮/暗/跟随系统 × 6 档强调色 = 48 种组合，此前只有暗/亮/自定义三档。
- 圆角、阴影、标题字族、动效速度全部进令牌，每套调色板有各自性格（霓虹 5px/130ms、暖陶 14px/220ms 衬线标题、紫曜 12px 紫辉光）；明暗与调色板解耦，每套都有亮暗两版。
- 强调色与实心填充色分离（`--accent` / `--accent-ink`），任何组合下白字压色块都保持可读；主题卡片是「用该主题自己的令牌渲染的小界面」，随亮暗实时换色。
- 首帧防闪内联脚本、切换时一次 220ms 统一过渡（不常驻）、减弱动效开关、自定义背景改为独立背景层（旧版自定义背景会把正文强制按暗色渲染）。
- 兼容旧数据：`settings.theme` 继续同步为 dark|light，`setTheme('dark'|'light'|'custom')` 仍可用，旧 `theme: "custom"` 自动迁移为背景层开启。

## 顶栏导航布局（电子书刊页同批）
- 媒体库从侧栏搬到顶栏平铺（原型 VaultHub-TopNav-Emerald）：标签即库名，带类型图标与条目计数、横向滚动；同组媒体库靠 `data-nav-key` 区分，不再出现「点电子书高亮漫画」。
- 顶栏一级导航只留首页 / 媒体搜索 / PT 管理，系统设置、外观主题、自定义模块、账户、退出登录收进右上角头像菜单（复用既有入口函数）。
- 顶栏全局搜索边输入边过滤当前库已渲染条目，回车转交「媒体搜索」页做全库检索，⌘K 聚焦。
- 布局可在「系统设置 → 外观主题 → 布局」切回侧边导航；顶栏模式下侧栏与拖拽把手整体让位，阅读器/播放器/弹窗遮罩左边界随 `--sidebar-w` 归零。
- 顶栏标签与侧栏共用同一份库数据与 `switchView` 高亮状态机；切回侧边导航时顶栏不再重复铺媒体库与主导航（侧栏已有同样入口），只留品牌、扫描/播放状态、搜索框与头像菜单。

## 电子书刊展示页重做
- 结构按原型：页头（库名 / 类型 / 本视图与全库条目 / 路径 / 上次扫描）+ 工具栏（书架标签 + 排序 + 密度 + 视图设置）+ 封面网格 + 分页。
- 书架视图升级为四标签：未读 / 喜欢 / 🕘 历史阅读 / 全部，标签带数量。
- 新增「喜欢」：封面心形按钮（悬停显形、已收藏常显），与音乐收藏同一套本地存储；点完心形标签上的计数就地刷新（不重载、不重打服务端），喜欢视图里取消收藏立即移出。
- 卡片新增「阅读 / 继续读 / 重读 ›」，元信息显示文件大小与进度；历史阅读仍保留 ↩ 释放。
- 排序按名称 / 添加时间 / 文件大小（服务端真实 path/size/mtime），密度 4/5/6 列循环，每页数量在视图设置里选。
- 分页默认关闭以保留 v0.9.70「完整展开」保证；开启后页码直跳、翻页纯本地切片（整库索引缓存 60 秒），不再打服务端。

## 顺带修掉的遗留缺陷
- `.audio-scrape-correction` 引用了从未定义的 `--line`/`--panel`，`var()` 无兜底导致整条声明失效：边框退回 currentColor、背景退回透明（v0.9.72 的刮削校正面板一直是无底色的亮描边）。已改用真实令牌。
- 播放器/视频控件里 34 处硬编码旧蓝色（`rgba(88,166,255)`、`#58a6ff`、`#0a84ff` 及透明变体）改为跟随主题的强调色与辉光。

## 验证
- Go `vet` + 全部 Go 测试（含 v0.9.73 的 5 个 `TestV0973*` 释放语义用例）；全部 Python 契约/行为测试；`node --check` 前端语法。
- 新增顶栏 + 书刊页契约测试：静态 48 项、Node VM 行为 44 项；15 条回滚突变全部被测试杀掉（含「顶栏不让位」「布局令牌丢失」「侧栏模式重复铺媒体库」）。
- Node VM 行为测试真跑：主题引擎 25 项（三层落盘/跟随系统实时切换/旧入口兼容/真预览卡片）、历史阅读释放与播放队列 22 项（DELETE 发出、服务端条目消失、本地残留清掉、书架即时少一张卡、索引左移）。
- 真实容器 + 浏览器实测（主题面板、播放列表面板、历史阅读释放前后书架对比），预览图随本次交付给出。
- 历史契约按新语义升级处均留注释（`setComicShelfView(comicShelfView)`、DELETE 成为释放动词、`scrollIntoView` 全量守卫）；历史发布说明与历史标签保持不可变。

详见 [RELEASE_NOTES_0.9.73.md](.github/RELEASE_NOTES_0.9.73.md)
