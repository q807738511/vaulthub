...
## 刮削状态、手工校正与源目录缓存
- 已添加媒体分别展示真实索引状态和刮削读取状态；失败/未匹配不再被文件名回退伪装成成功。
- 「已刮削」判定要求文件仍在索引中，且磁盘实际大小/修改时间与记录一致；重新打标签、替换或删除文件后旧结果立即失效。
- 音频编辑增加独立查询参数、地区/来源选择和字段锁；自动与手工结果统一缓存到 SQLite，并写入 `<stem>.vaulthub.json`、`.lrc`、`.cover.*` sidecar。
- 只读目录明确返回仅服务端缓存，拒绝覆盖用户自建 sidecar 与符号链接目标；同路径写入进程内串行，避免并发覆盖。
- 旧版缓存写入口改为 UPSERT，不再清空本版的检索词、地区、来源、状态与字段锁；缓存读回过期条目不再返回，NULL 列不再导致整行被静默丢弃。

## 性能与残留修复
- 音频元数据采用会话内内存缓存，避免列表渲染反复解析 localStorage；不可见页面暂停轮询，状态不变不重建表格，离屏卡片延迟布局。
- 刮削状态带 10 秒进程内快照（首页 5 秒轮询不再反复全量 stat），元数据写入/删除后立即失效。
- 删除 weakProbe 不可达 HEAD 分支；加固 ffmpeg 路径与错误输出上限。
- 0.9.71 测试计数口径在本版纠正；已弃用 VaultHub.env 保持不使用。

## 审查与自查修复
- 独立审查发现并修复：失效元数据虚增完成度、同路径 sidecar 并发覆盖、旧写入口清空修正字段。
- 自查发现并修复：新鲜度只比对索引表（磁盘改动未重扫时仍算完成）、缓存读回 NULL 列静默丢行。
- 6 项突变验证全部被测试捕获。

# VaultHub 蜀鼠之家 v0.9.71：刮削识别与弱网方案

- 音乐刮削识别策略调整：iTunes 店铺按查询脚本动态选择（含假名→JP 优先、汉字→TW 优先、拉丁→US 优先，最多 3 家且满分即停）；检索词改为阶梯（原样 → 去修饰 → 主歌手 → 仅标题）；全角/半角统一折叠；歌手封面用 MusicBrainz 别名表桥接罗马音规范名（日语歌手此前必然刮不到）。
- 歌词取源改为阶梯：本地 .lrc/内嵌标签 → LRCLIB get/search 多变体 → 自由文本 q= 兜底 → lyrics.ovh；并停止「匹配不上就贴列表第一条有歌词记录」的旧行为，新增去修饰与脚本一致性判定，避免贴错歌词。
- 放大视图右侧新增「海报 / 歌词」药丸；歌词在页面中部展示、可拖动浏览（松手 6 秒内暂停自动跟随）、点击行跳到该句播放；主海报按宽度 5% 虚化（CSS blur 不接受百分比，按海报宽度折算）。
- 新增弱网方案：服务端限码率音频流（96k–320k，转码落盘 + Range + 私有缓存，配额独立于漫画页缓存）、下行测速端点、播放器音质药丸、系统设置「弱网模式」（自动/始终/关闭）与一键测速、漫画省流联动；客户向手册 `docs/weak-network-playbook.md`。
- 附带修复：漫画单页模式第二槽位未隐藏（作者样式 `.comic-slot{display:flex}` 盖掉 UA 的 `[hidden]{display:none}`，表现为正文半宽 + 右侧空槽）。
- 独立审查后发现并修复：自由文本 q= 兜底在 4 个检索词变体时跑不到（改分阶段外呼预算）、短拉丁标题被更长无关标题包含时误判（Rain ⊂ Rain On Me 拿错歌词）、真实标题前导数字被当音轨号裁掉（21 Guns → Guns）、同键并发重复转码（改 singleflight）、HEAD 请求触发完整转码、外呼上限未覆盖 lyrics.ovh；海报 5% 虚化改为随断点折算。
- 版本 0.9.71。

详见 [RELEASE_NOTES_0.9.71.md](.github/RELEASE_NOTES_0.9.71.md)

# VaultHub 蜀鼠之家 v0.9.70：阅读器与已读收藏修复

- 修复漫画/电子书已读收藏视图重新展开失败：改用完整索引后再过滤已读/未读，避免 `limit=100000` 截断。
- 新增 EPUB 正文解析（标准库自解析 ZIP 容器，按 spine 顺序生成章节），PDF/TXT/图片阅读路径保持登录保护；解析设条目、正文、章节与扫描量硬上限。
- 完成历史遗留焦点、播放器布局、PDF 路径和版本一致性检查。
- 独立审查后修复：EPUB 跳过 script/style 时吞掉整章正文、章节下拉跳转算出 NaN、章节标题字节错位、发布说明表述与实现不符、注释技术论断被证伪、提交内误入 `.pyc`；并补齐扫描预算与 >500 本完整索引守卫。
- 网易云歌词/元数据源不启用。
...
阻塞项与首批 3 条非阻塞项已随 v0.9.67 修掉；本补丁把**剩余 9 条非阻塞项**逐条修掉并补回归测试：

- 受鉴权的转码结果缓存头 `public` → `private`（避免中间缓存向未登录者重放）。
- 缓存键纳入**归档自身** size/mtime（原用条目元数据，重打包保留时间戳会下发陈旧页）。
- 封面端点强制缩略：`w=0` → 400、不可转码 → 404（不再回整张原图）。
- 转码 leader 用 `defer`+`recover` 保证任何路径都结束 job（原 panic 会让同键永久阻塞）。
- 歌词识别与落盘限定**音频库 + 音频扩展名**；sidecar 用不可预测临时名 + 拒绝符号链接目标。
- 批量歌词上限 200 → 50 并加 150s 预算（原单次可占用连接约 9 分钟）。
- 缓存淘汰改为锁内摘表、锁外 unlink；清空整库缓存需显式 `?all=1`。
- 前端 `setPlaybackBg` 封面 URL 经 `cssUrlValue` 清洗（服务端元数据缓存可被写入）。

明示未改：音频缓存 mtime 保持秒级（JOIN 依赖同精度，改纳秒会导致全部重刮）。

详见 [RELEASE_NOTES_0.9.68.md](.github/RELEASE_NOTES_0.9.68.md)

---

# VaultHub 蜀鼠之家 v0.9.67：漫画阅读器重构 · 音乐歌词刮削与元数据缓存

v0.9.67 合并两块改造（均不新增容器）：

- **漫画阅读器重构**：实测定位「慢」的真因（单页 1.5–3.8MB 原图 + 零预取 + 缓存 1 小时；归档 95% 是 PNG 无损扫图，服务端解压仅 33ms/页，并非瓶颈）；新增按页转码端点与首图封面端点；阅读器支持单页/双页/条漫、右起/左起、适宽/适高/原始、预取 ±3 页、**按页码记进度**、页码跳转、键盘与半屏点击；「省流模式」默认自动（弱网才转码，局域网原图直出更快）。
- **音乐歌词刮削**：新增本地识别（同名 .lrc、ID3 USLT、Vorbis LYRICS、MP4 ©lyr）与在线源链（LRCLIB → lyrics.ovh → 可选网易云，带 503 退避与占位内容过滤），命中后落盘同目录 `<名>.lrc`（原子写，不改音频文件）。
- **刮削源扩展**：源链加入（可选）网易云搜索与 Cover Art Archive 封面兜底，提高华语/冷门曲目命中率。
- **服务端元数据缓存**：新增 sqlite 表按 size+mtime 失效，换浏览器/清缓存后不再重复联网刮削。

详见 [RELEASE_NOTES_0.9.67.md](.github/RELEASE_NOTES_0.9.67.md)

---

# VaultHub 蜀鼠之家 v0.9.66：海报遮罩左缘对齐修正 · 无障碍与状态归一化

v0.9.66 修正 v0.9.65 发布后发现的非阻塞问题：

- **遮罩左缘对齐侧栏实际占位**：桌面折叠态是 62px 图标轨（`.main` 同样 margin-left:62px），遮罩改从 62px 起，不再压住图标轨；拖拽时跟随 `--sidebar-w`，窄屏侧栏 0 宽时回到 0。
- **放大态归一化**：离开音乐视图即关闭海报放大态，切回不再自动弹出全屏海报。
- **无障碍**：遮罩补 `role="dialog"` / `aria-modal` / `aria-label`，并提供可聚焦的视觉隐藏关闭位与焦点归还。
- **层级与文案**：遮罩 z-index 650 → 独立 660；澄清「放大后遮罩高于播放器，放大按钮不可点，返回请点击遮罩或按 Esc」。

详见 [RELEASE_NOTES_0.9.66.md](.github/RELEASE_NOTES_0.9.66.md)
...
- 移除专辑/歌手海报卡片上重复的 ✎ 编辑按钮，编辑入口统一放在曲目列表头部。
- 移除最大化海报右上角返回按钮（点击遮罩任意位置或 Esc 返回）；放大按钮在封面内自适应居中，左侧专辑海报尺寸 clamp 自适应。
- 侧栏折叠时遮罩左缘归零，完整覆盖侧栏伸缩后让出的空缺。
- 移动端侧栏的媒体库/自定义容器改为 display:contents，全部页面项可横向滚出。
- 海报放大四周磨砂清晰度调整为 85%（模糊半径 42px → 14px）。

详见 [RELEASE_NOTES_0.9.65.md](.github/RELEASE_NOTES_0.9.65.md)

---

# VaultHub 蜀鼠之家 v0.9.64：媒体播放器整体修复

v0.9.64 汇总修复 v0.9.59—v0.9.63 期间反馈的播放器交互与背景问题：

- 音乐最大化按钮固定到左侧封面，默认隐藏、悬停显示；放大后播放器原位保留，清晰海报画中画，画外使用当前封面 90% 磨砂遮罩，右上角返回。
- 影视、剧集、歌曲详情页使用当前查看条目的图片作为 95% 背景，不再持续复用正在播放媒体的背景。
- 音频与视频共享唯一播放权，切换类型时同步暂停另一播放器并释放保活，杜绝同时播放。
- 视频向下缩放后停靠到底部控制器一排，控制器持续显示，视频缩到控制器左下角。

详见 [RELEASE_NOTES_0.9.64.md](.github/RELEASE_NOTES_0.9.64.md)

---

# VaultHub 蜀鼠之家 v0.9.61：全屏海报遮罩层级修复 · 移除旧最大化按钮

v0.9.61 修复了音乐播放器全屏海报放大功能被顶栏/侧边栏遮挡的问题，并清理了旧版最大化入口：

- **全屏海报遮罩层级修复**：海报放大遮罩 `z-index` 由 280 提升至 650，高于顶栏与侧边栏（600），放大后海报不再被侧边栏/顶栏遮挡，右上角返回按钮可正常点击。
- **移除控制栏旧「最大化」按钮**：删除播放器控制条中的旧最大化按钮及其居中放大逻辑（旧播放器 `z-index:250` 低于侧边栏 600 且 `left:50%` 居中会侵入侧边栏区域导致遮挡），海报放大统一通过左侧封面悬停显示的放大按钮进入。
- 歌词双页（海报/歌词）DOM 与逻辑保留，详情弹窗歌词查看不受影响。

详见 [RELEASE_NOTES_0.9.61.md](.github/RELEASE_NOTES_0.9.61.md)
...
详见 [RELEASE_NOTES_0.9.60.md](.github/RELEASE_NOTES_0.9.60.md)

---

# VaultHub 蜀鼠之家 v0.9.59：媒体播放会话保活 · 长时间播放不再自动登出

v0.9.59 修复长时间播放音乐或视频时，因页面没有点击/滚动等交互而在 30 分钟后自动登出、后续媒体请求返回 401 的问题，并增加封面海报全屏放大功能：

- **前端 idle 保活**：音频/视频实际播放期间，每 5 分钟刷新一次前端 idle 定时器，不再触发 `/api/logout` 主动杀掉自己的会话。
- **服务端滑动会话**：同一周期请求受保护的 `/api/system/runtime`，延长服务端 30 分钟滑动会话；401/403 仍交给统一鉴权逻辑处理，不绕过登录。
- **完整生命周期**：音频停止/错误、视频结束/错误/关闭时释放保活；具名 `Set` 保证连续切歌与视频切流不会重复累计定时器，音频与视频可独立启停。
- **短时断网容错**：保活请求失败不打断正在播放的媒体，下一周期自动重试。
- **封面海报全屏放大**：底部播放器封面左上方新增放大按钮（默认隐藏，鼠标悬停渐显），点击后专辑海报以全屏遮罩占满整个音乐界面，播放器位置不变；遮罩右上角提供返回按钮，点击收起。无封面时显示歌曲标题文字回退，亮色主题同步适配。

详见 [RELEASE_NOTES_0.9.59.md](.github/RELEASE_NOTES_0.9.59.md)

<details>
<summary>v0.9.58：安全修复 · 随机初始密码与强制改密 · 增强密码校验（上一版）</summary>
...
- **影视详情返回按钮药丸化（修复穿模）**：详情页右上角「✕ 返回详情 / ✕ 返回媒体库」此前是圆形按钮，放不下长文案导致文字溢出重叠；改为**横向药丸**（SVG 返回箭头「←」+ 中文），宽度随文案自适应、圆角胶囊样式、hover 高亮；阅读器的圆形 ✕ 关闭按钮不受影响。
- **音乐专辑/歌手操作逻辑补齐**：专辑/歌手卡片新增「▶ 播放」—— 不进入列表**直接播放该分组全部歌曲**；点进分组后的曲目列表头部新增「▶ 播放全部」一键直播；分组内点击任一曲目**立即播放且队列即该专辑/歌手**（上一首/下一首/自动连播都在分组内）；点击专辑/歌手卡片展示该分组全部曲目（整单拉取，不再只显示第一页，未知专辑/未知歌手同样可打开）。
- **TXT 解码边界修复（审查 #1）**：无 BOM 纯 CJK UTF-16 文件零字节密度趋近 0，个别字节对又恰好构成合法 UTF-8，会被严格分支抢先解成乱码；新增 `probeUtf16Pairing` 奇偶位汉字高字节占比配对探测，命中后与严格 UTF-8 + 全部候选**全量打分择优**，新增 `话还这过试` 类常用字 UTF-8 伪装边界样本（解码矩阵 25/25）。
- **歌手匹配阈值与共享字符去重（审查 #2/#4b）**：`itunesPickArtist` 共享非拉丁字符门槛 1→2，与歌曲侧一致，`邓丽君/邓紫棋` 式仅共享一字不再误配，简繁「周杰倫/周杰伦」仍正确命中；`sharedCommonAscii` 只按不同字符计数。
- **itunesPick 回落阶梯显式化（审查 #3）**：新增「宽松标题 + 歌手校验」第二趟 —— 带 `(Album Version)` 后缀的自家版本优先于精确标题翻唱，第三趟保留精确标题兜底，回归测试固定优先级。
- **歌手改名保留合作参与者（审查 #4a）**：批量编辑歌手名不再重置刮削出的 `collaborators`；封面未改时保留原封面。
...
# VaultHub 蜀鼠之家 v0.9.56：TXT 阅读编码修复 · 音频刮削修正与歌手刮削 · 专辑/歌手可编辑与喜欢栏目 · 播放器喜欢按钮与居中 · 移动端顶栏滑动 · 剧集图示卡片

v0.9.56 修正了 TXT 阅读乱码与音频刮削错配，并把音乐库的歌手维度、收藏与播放器交互补齐：

- **TXT 文档阅读乱码修复**：旧实现只有「UTF-8 失败就当 GB18030」两条路，Windows 记事本存的 UTF-16、繁体 Big5、日文 Shift-JIS 一律解成乱码。现在按 **BOM → 无 BOM UTF-16 零字节特征 → UTF-8 严格 → 多候选打分**（gb18030/big5/shift-jis/euc-kr/utf-16le/utf-16be）判定，12 组真实编码样本与阅读器链路全部无乱码。
- **音频刮削修正**：iTunes 端点纠错（旧代码实际请求 `/search/search`，歌手 `lookup` 拼成 `/search/lookup` 永远空结果）；合作演唱曲目改为「标题 + 歌手」择优，`周杰伦 feat. 费玉清 - 千里之外` 不再被翻唱专辑抢走；整串查不到时用主歌手重试；**刮削失败不写伪造元数据，直接以文件名解析结果展示**。
- **新增歌手刮削 + 专辑/歌手可编辑**：`/api/media/audio/artist`（iTunes musicArtist → 代表专辑封面 600x600 → MusicBrainz 兜底），合作关系拆出全部参与者并标注「合作演唱」；专辑/歌手卡片新增 ✎ 批量编辑（改名/换封面写入该分组全部曲目，标记 manual 后不被自动刮削覆盖）。
- **点击专辑/歌手进入整单曲目 + 喜欢栏目**：卡片整卡点击展示该专辑/歌手的**全部曲目**（此前分页只取第一页），曲目行整行点击直接播放且连播队列即当前列表；新增 `♥ 喜欢` 页签汇总收藏曲目。
- **播放器喜欢按钮 + 居中显示**：播放器控制条新增 ♥ 按钮并与列表收藏状态同步；底部条水平居中，展开态上下左右居中。
- **移动端顶栏可滑动查看更多选项**：scroll-snap 停靠 + 触摸惯性滚动，内容溢出时右侧渐隐提示「还有更多」，滑到最右端自动消失。
- **剧集列表图示卡片化**：剧集详情内单集改为缩略图 + 集号角标卡片，整卡点击直接播放，移动端自适应缩小。
- **README 与更新日志分离**：原更新日志 `README.md` 更名为 `Update Log.md` 归档；主页 `README.md` 改为项目介绍。

详见 [RELEASE_NOTES_0.9.56.md](.github/RELEASE_NOTES_0.9.56.md)
</details>

<details>
<summary>v0.9.55：移动端顶栏化 · 密码验证鉴权加固 · 文件读取鉴权与下载权限清除 · 开放模式登录窗口移除（上一版）</summary>

# 体验链接(开启了开放模式)https://home.example.com
